...
async def get_sparklines(
    source_ids: str = Query(..., description="ID источников через запятую"),
    ctx=Depends(get_workspace_context),
    session: AsyncSession = Depends(get_session),
):
    """Batch sparkline данные — количество постов/день за 7 дней для нескольких источников."""
    ids = [s.strip() for s in source_ids.split(",") if s.strip()]
    data = await storage.get_sparklines_batch(session, ids)
    return data
...
async def trigger_parse(source_id: str, ctx=Depends(get_workspace_context), session: AsyncSession = Depends(get_session)):
    """Ручной запуск парсинга источника (фоновый) с rate-limit и проверкой дублей."""
    source = await source_manager.get_source(ctx["user_id"], source_id)
    if not source:
        raise HTTPException(404, "Source not found")
    # Rate-limit: не чаще 1 раза в 5 минут
    if source.get("last_parsed_at"):
        last = datetime.fromisoformat(source["last_parsed_at"])
        if last.tzinfo is None:
            last = last.replace(tzinfo=timezone.utc)
        cooldown = timedelta(minutes=5)
        diff = datetime.now(timezone.utc) - last
        if diff < cooldown:
            remaining = int((cooldown - diff).total_seconds())
            raise HTTPException(429, f"Cooldown: повторный запуск через {remaining} сек")
    # Проверка: нет ли уже запущенного парсинга
    if await storage.has_running_run(session, source_id):
        raise HTTPException(429, "Парсинг уже запущен для этого источника")
