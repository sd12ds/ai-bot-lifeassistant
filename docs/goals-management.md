...
> Версия: 1.1 | Дата: 2026-03-15 | Обновлено: реализация cf9efa3
...
### 6.1 Что отображается сейчас (актуально после cf9efa3)

- **Шапка**: название + область (emoji + label) + статус-бейдж (🟢/❄️/🔴/✅) + дедлайн с цветом + плашка заморозки с `frozen_reason`
- **Блок «Зачем мне это»**: `why_statement`
- **Блок «🚀 Первый шаг»**: `first_step` (если заполнен)
- **Прогресс**: анимированный прогресс-бар + «X из Y этапов»
- **Воронка этапов**: нумерация 1→N с коннекторами, `due_date` с цветом, % вклада в цель, inline-форма добавления, пустое состояние с подсказкой §12.2
- **Привязанные привычки**: список по `goal_id` + streak + статус сегодня
- **История чекинов**: последние 5, фильтр по `goal_id`
- **AI-инсайт**: если `goal.ai_insight` не пустой
- **Bottom sheet «📈 Обновить прогресс»**: слайдер 0–100% шаг 5, `coaching_notes`, предложение «Отметить достигнутой?» при 100%
- **Sticky-панель**: `[📈 Прогресс]` `[➕ Этап]` `[⋯]` → шторка: Перезапустить / Заморозить / Возобновить / Достигнуто

### 6.2 Нереализованные функции (Phase 2)

Всё из предыдущей таблицы реализовано в cf9efa3. Остаётся Phase 2:

| Функция                            | Статус      | Примечание                                    |
|------------------------------------|-------------|-----------------------------------------------|
| AI-генерация milestone'ов          | ⏳ Phase 2  | `POST /goals/{id}/generate-plan` не реализован |
| Редактирование title/area/deadline | ⏳ Phase 2  | Отдельная задача (inline-edit)                |
| Inline-чекин по цели               | ⏳ Phase 2  | `POST /goals/{id}/checkin` — отдельная задача |

---

## 7. Реализованные изменения (коммит cf9efa3, план 83ae6009)

### 7.1 Шапка — мета-блок цели

**Было:** только название + кнопка назад.
**Стало:**
...
**Было:** простой список checkbox, нет дат, нет добавления.
**Стало:**
...
## 12. История изменений GoalDetailPage

### v1.0 (исходное состояние)
- read-only прогресс-бар
- список этапов только с отметкой выполнения
- sticky-панель: только Заморозить / Достигнуто / Возобновить

### v2.0 — коммит cf9efa3 (2026-03-15)
**Бэкенд:**
- `GET /coaching/habits` — добавлен query-param `goal_id`

**`coaching.ts`:**
- `useCreateMilestone()` — POST /milestones
- `useRestartGoal()` — POST /goals/{id}/restart
- `useHabitsByGoal(goalId)` — GET /habits?goal_id=

**`GoalDetailPage.tsx`:**
- Шапка: area emoji + статус-бейдж + дедлайн с цветом + frozen_reason плашка
- Блок «🚀 Первый шаг»
- Воронка этапов: нумерация, due_date, % вклада, inline-форма, пустое состояние
- Секция привязанных привычек: streak + статус сегодня
- Bottom sheet «📈 Обновить прогресс»: слайдер + заметка + offer-achieve
- Sticky-панель: [Прогресс] [+ Этап] [⋯] + шторка дополнительных действий
- Диалог подтверждения перезапуска

### Phase 2 (не реализовано)
- AI-генерация milestone'ов через LLM
- Inline-редактирование полей цели
- Чекин прямо по цели из детального экрана

---

> Документ создан: 2026-03-15 | Обновлён: 2026-03-15
> Статус реализации: план 83ae6009 выполнен полностью (cf9efa3)
