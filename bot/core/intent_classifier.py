...
_RESEARCH_STRONG = {
    ...
    # Управление задачами research (только уникальные, без конфликтов с другими доменами)
    "результаты сбора", "статус сбора", "список задач сбора",
    "запусти сбор", "отмени сбор", "покажи результаты парсинга",
    "результаты research", "задачи сбора",
}
...
_RESEARCH_NORMAL = {
    ...
    "результат", "сколько найдено", "что нашел",
}
