...
v3: пятиуровневая классификация:
  0a) Draft Guard — активный черновик → агент черновика (без LLM)
  0b) Confirmation Guard — ожидание подтверждения + да/нет → текущий домен (без LLM)
  1)  force_agent из обработчика
  2)  Rule-based pre-classifier (keywords, без LLM)
  3)  Sticky domain (8 мин после последнего взаимодействия)
  4)  LLM-классификатор (gpt-4.1-nano) с контекстом active_domain
"""
...
# Паттерн для Confirmation Guard (L0b) — однословные подтверждения и отмены
...
async def classify_intent(state: SupervisorState) -> SupervisorState:
    """Определяет тип агента: L0a draft → L0b confirm → force_agent → rule-based → sticky → LLM."""
    # ── Уровень 0: Защита черновиков и подтверждений (defence-in-depth) ─────────
    # Срабатывает ДО force_agent, ДО rule-based, ДО LLM.
    # Гарантирует корректный routing при любом источнике сообщения (text/voice/photo).
    from bot.core.session_context import get_context as _sc0
    _uid0 = state["user_id"]
    _ctx0 = _sc0(_uid0)
    # 0a: активный черновик → всегда идём к агенту черновика
    if _ctx0 and _ctx0.draft:
        _a0 = _ctx0.active_domain or "nutrition"
        _last_agent_per_user[_uid0] = _a0
        logger.info("user=%s → %s (L0a draft guard)", _uid0, _a0)
        return {**state, "agent_type": _a0}
    # 0b: ожидаем подтверждения + однословный ответ → к текущему домену
    _msg0 = next((m for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), None)
    if _ctx0 and _ctx0.pending_confirmation and _msg0 and _CONFIRM_PATTERN.match(_msg0.content.strip()):
        _a0 = _ctx0.active_domain or "nutrition"
        _last_agent_per_user[_uid0] = _a0
        logger.info("user=%s → %s (L0b confirmation guard)", _uid0, _a0)
        return {**state, "agent_type": _a0}
    ...
    if rule_result:
        ...
        if _ctx_rb:
            ...
            _ctx_rb.activate_sticky(minutes=8)
...
async def process_message(
    ...
    """
    # ── Слой 1: Авто-force из активного черновика ─────────────────────────────
    # Гарантирует корректный routing ДЛЯ ЛЮБОГО обработчика (text, voice, photo...)
    # без необходимости добавлять draft-логику в каждый handler отдельно.
    if not force_agent:
        ...
        if _ctx1 and _ctx1.draft:
            ...
            logger.info("process_message user=%s: авто-force → %s (Layer 1)", user_id, force_agent)
